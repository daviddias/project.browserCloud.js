...
\begin{table}
  \begin{tabular}{| p{1.3cm} | p{1.6cm} | p{1.7cm} | p{1.8cm} | p{1.6cm} | p{1.8cm} | p{1.8cm} |}
    ...
    \hline  
    Kademlia & Binary tree & XOR metric & x=current nodeId, y=target nodeId, d=distance between two nodes (d(x,y)), n=number of nodes, m=number of different bits (prefix) & O(log(n)) & \textsubscript{2}(n) & not stable \\
    \hline      
  \end{tabular}
