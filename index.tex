...
\begin{abstract}
Grid computing has been around since the 90\'s, it's fundamental basis is to use idle resources in geographically distributed systems in order to maximize it's efficiency, giving researches access to computational resources to perform their analysis. This approach quickly grew into non grid environments, causing the appearance of projects such as SETI@Home or Folding@Home, using volunteered shared resources and not only institutionalized data centers, creating the concept of Public Computing. Today, after having this concepts proven as capable of being performant, we face the challenge of how to give a one, simple way of people to participate in this community efforts and even more importantly, how to reduce the cost of adoption by developers and researchers to use this resources for their applications. This work explores current ways of making an interopable way of end user machines to communicate, using new Web technologies, creating a simple API that's familiar to those used to develop applications for the Cloud, but that resources are providing by a community and not by a company.
...
\begin{table}
  \begin{tabular}{| p{1.3cm} | p{1.6cm} | p{1.7cm} | p{1.8cm} | p{1.6cm} | p{1.8cm} | p{1.8cm} |}
    \hline                        
