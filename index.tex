...
Human's Cloud storage happens in what it's called, the ``Ascended node ring'', this nodes 

% What is stored:
 % object chunks
 % object metadata (location of chunks)
 % reputation log
 % directories metadata (/home), so hcls works

% Power of 2 choices -> 1 node has pointers and is the skeeper of the file, replicated of course

% how chunking is done (only the keeper, makes the chunking hashing), but data get's chunked as soon as it enters the network, to avoid transmitting files of of XYZ Gb's from Node to Node, and also, it goes out of the network unchunked 
...
% Load balancing through virtual servers technique 
