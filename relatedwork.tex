% 
% Related work
% 

\section{Related Work}
The purpose of this section is to show the state of the art of the research topic, namely: Volunteer Computing, Cloud Computing, P2P Networks and the Web Platform
%%TODO: Describe what's coming stating section numbers



%---------------------{Volunteer Computing}----------------------
  \subsection{Volunteer Computing}
% Here is more about projects, not about algorithms (SETI, freenet, etc)


    \subsubsection{Cycle Sharing}

    \subsubsection{Storage Sharing}


%---------------------{Cloud Computing}------------------
  \subsection{Cloud Computing}
% different methodologies 



%---------------------{Peer-to-peer(P2P) Networks}---------------
  \subsection{Peer-to-peer(P2P) Networks}
% ALL THE P2P algorithms (Unstructured/Structured)

    \subsubsection{Peer-to-peer Architectures}

      \subsubsubsection{Unstructured}

        \subsubsubsubsection{Deterministic}
        %Napster, BitTorrent, JXTA


        \subsubsubsubsection{Non-Deterministic}
        %Gnutella, Kazaa

      \subsubsubsection{Structured}

        \subsubsubsubsection{Distributed Hash Tables}
        %CAN ;CHORD ;TAPESTRY; PASTRY; Kademlia

        \subsubsubsubsection{Non-Distributed Hash Tables}
        %Mercury

      \subsubsubsection{Hybrid}
      %Structella , Kelip

    
    \subsubsection{Peer-to-peer Applications}
    %Kazaa, napsert, Seti, foldings, afins


    \subsubsection{Assurance and Trust}
    % Fault tolerance
    % Threshold Cryptography
    % Reputation Management
    % Economic Models




%---------------------{Web Platform}-----------------------------
  \subsection{Web platform} 
  % Changed in the web Platform


    \subsubsection{What has been happening}
    %Javascript
    %WebRTC
    %HTTP 2.0

    \subsubsection{Previous attempts}
    %Merelo and friends
