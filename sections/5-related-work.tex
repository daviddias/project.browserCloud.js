...
One of the main focuses with the proposed work, is to take advantage more recent developments of the Web platform, the viability of system depends on system components such as:
\begin{itemize}
  \item High dynamic run time for ongoing updates of the platform and specific assets for job execution
  \item Close to native performance for high CPU bounded jobs
  \item Peer-to-peer interconnectivity
  \item Scalable storage and fast indexing
\end{itemize}

We explore in this section the components present or undergoing a development process for the Web platform, such as: Javascript, Emscripten, IndexedDB, WebRTC and HTTP2.0, these enable the necessary features to such a distributed shared resource system:
